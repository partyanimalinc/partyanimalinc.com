...
export function AdventVideoGallery() {
  ...
  const onTouchEnd = (e: React.TouchEvent) => {
    ...
    touch.current = null;
    if (Math.max(Math.abs(dx), Math.abs(dy)) < 45) return; // treat as tap → play/pause
    ...
    <>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4">
        {ADVENT_VIDEOS.map((v, i) => (
          ...
          >
            <div className="relative aspect-[9/16] overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] shadow-lg transition-all duration-200 group-hover:-translate-y-1 group-hover:border-brand-red/50 group-hover:shadow-2xl">
              ...
              <span className="absolute left-1/2 top-1/2 grid h-14 w-14 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full bg-brand-red/90 text-white shadow-xl transition-transform duration-200 group-hover:scale-110">
                <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
                  <path d="M8 5.5v13l11-6.5-11-6.5Z" />
                </svg>
              </span>
              <span className="absolute inset-x-0 bottom-0 p-3 font-heading text-sm uppercase leading-tight text-white drop-shadow">
                {v.title}
              </span>
            </div>
          </button>
      ...
      {open && active && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/92 backdrop-blur-sm">
          ...
          <div
            ...
            className="relative aspect-[9/16] h-[100dvh] w-full overflow-hidden bg-black sm:h-[88vh] sm:w-auto sm:rounded-[2rem] sm:ring-1 sm:ring-white/15 sm:shadow-[0_30px_80px_-20px_rgba(0,0,0,0.9)]"
          >
            <Stream
              key={active.uid}
              streamRef={player}
            ...
            {/* tap layer: play / pause (sits under the chrome) */}
            <button
              type="button"
              onClick={togglePlay}
